...
def get_word_usage(read_file,new_file, encoding="utf-8"):
    with open(read_file, encoding=encoding) as fin, open(new_file, mode="w", encoding=encoding) as fout:
        ...
        max_frequency=most_frequent_word_list.most_common(1)
        
        for x in max_frequency:
            print(x) 
        # max_frequency = 0
        # if most_frequent_word_list[text] > max_frequency:
        #     max_frequency = most_frequent_word_list[text]
        # fout.write(max_frequency)
